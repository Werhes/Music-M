...
namespace ELOR.Laney {
    public sealed class App : Application {
            ...
            foreach (string arg in args) {
                if (arg.StartsWith($"-{key}")) {
                    ...
                    if (idx >= 0 && idx < arg.Length - 1) {
                        // Trim surrounding double quotes, so paths with spaces can be passed safely.
        ...
        /// <summary>
        /// Music M "messenger-mode" integration: provisions the account token and user id
        /// (passed via command line as -token=... -userid=...) into the encrypted settings
        /// file using Laney's own encryption, before the login check runs. This lets the
        /// messenger open already signed in with the current Music M account.
        /// </summary>
        private static void ProvisionTokenFromCommandLine() {
            try {
                string token = GetCmdLineValue("token");
                if (String.IsNullOrEmpty(token)) return;

                long uid = 0;
                Int64.TryParse(GetCmdLineValue("userid"), out uid);

                var batch = new Dictionary<string, object> {
                    { Settings.VK_TOKEN, token }
                };
                if (uid > 0) batch[Settings.VK_USER_ID] = uid;

                Settings.SetBatch(batch);
                Log.Information("Token provisioned from command line (Music M messenger-mode).");
            } catch (Exception ex) {
                Log.Error(ex, "Failed to provision token from command line!");
            }
        }
        ...
        public override void OnFrameworkInitializationCompleted() {
            if (ApplicationLifetime is ClassicDesktopStyleApplicationLifetime desktop) {
                ...
                if (Program.Mode == LaunchMode.Default) {
                    // Music M "messenger-mode": sign in with the token passed from Music M.
                    ProvisionTokenFromCommandLine();
        ...
        private static string GetUserAgent() {
            string ver = BuildInfoFull;
            string[] sections = ver.Split('-');
            return $"MusicMMessenger (2; {sections[0]}; {sections[1]}; {sections[2]})";
        }
        ...
        private static string GetLocalDataPath() {
            ...
            if (!string.IsNullOrEmpty(custom) && Path.EndsInDirectorySeparator(custom)) {
                ...
                return Path.Combine(appdataroot, "MusicM", "Messenger");
            }
        }
